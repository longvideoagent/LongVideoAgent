...
def bbox_to_string_simplified(file_path: str | None = None, key=None):
    """
...
def main():
    ...
    parser.add_argument("--bbox-json-path", type=str, default=DEFAULT_BBOX_JSON_PATH, help="bbox JSON 路径")
    ...
    if not Path(args.bbox_json_path).is_file():
        raise FileNotFoundError(f"BBox JSON file not found: {args.bbox_json_path}")
    if args.threads <= 0:
